...
export default function AdminAnalyticsPage() {
  ...
  if (loading) return <div style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 14, color: '#4F5568', padding: 40 }}>Loading analytics...</div>

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 style={{ fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 900, fontSize: 28, color: '#fff', margin: 0, textTransform: 'uppercase' }}>Analytics</h1>
        <div style={{ display: 'flex', gap: 4 }}>
          {(['7d', '30d', '90d'] as Period[]).map(p => (
            <button key={p} onClick={() => setPeriod(p)} style={{
              padding: '5px 12px', fontSize: 10, fontWeight: 700, fontFamily: 'Rajdhani, sans-serif',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        ...
        <div style={{ background: '#13131E', border: '1px solid rgba(255,255,255,.06)', borderRadius: 10, padding: 16 }}>
          <div style={{ fontSize: 10, fontWeight: 800, fontFamily: 'Rajdhani, sans-serif', color: '#4F5568', textTransform: 'uppercase', letterSpacing: .8, marginBottom: 10 }}>
            Top 10 Earners
          </div>
          ...
          {(!Array.isArray(topEarners) || topEarners.length === 0) && <div style={{ fontSize: 10, color: '#4F5568', fontFamily: 'Rajdhani, sans-serif', textAlign: 'center', padding: 12 }}>No data</div>}
        </div>
        ...
        <div style={{ background: '#13131E', border: '1px solid rgba(255,255,255,.06)', borderRadius: 10, padding: 16 }}>
          ...
          {(Array.isArray(topGames) ? topGames : []).map((g: any, i: number) => (
            <div key={g._id} style={{
              ...
              <span style={{ fontSize: 12, fontWeight: 900, color: i < 3 ? '#3b82f6' : '#4F5568', fontFamily: 'Barlow Condensed, sans-serif', width: 20 }}>#{i + 1}</span>
              <span style={{ fontSize: 11, fontWeight: 700, color: '#DDE0EA', fontFamily: 'Rajdhani, sans-serif', flex: 1 }}>{g._id}</span>
              <span style={{ fontSize: 11, fontWeight: 700, color: '#8890A4', fontFamily: 'Rajdhani, sans-serif' }}>{g.count} matches</span>
            </div>
          ))}
          {(!Array.isArray(topGames) || topGames.length === 0) && <div style={{ fontSize: 10, color: '#4F5568', fontFamily: 'Rajdhani, sans-serif', textAlign: 'center', padding: 12 }}>No data</div>}
        </div>
      </div>
    </div>
  )
