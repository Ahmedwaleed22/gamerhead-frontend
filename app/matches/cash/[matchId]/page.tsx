...
// FILE: app/matches/cash/[matchId]/page.tsx
...
const ACCENT      = '#F0AA1A'
const ACCENT_DIM  = 'rgba(212,146,10,.15)'
const ACCENT_BDR  = 'rgba(212,146,10,.25)'
...
// ─── PAGE ──────────────────────────────────────────────
export default function CashMatchPage() {
  ...
  const wagerPerPlayer = match?.wagerPerPlayer || 0
  const totalPot       = match?.totalPot || 0
  const winnerPayout   = match?.winnerPayout || 0
  const platformFee    = match?.platformFee || 7.5
  const playerCount    = (teamA.players.length + teamB.players.length) || 10
  const potDisplay     = totalPot ? `$${(totalPot / 100).toFixed(2)}` : '$0.00'
  const entryDisplay   = wagerPerPlayer ? `$${(wagerPerPlayer / 100).toFixed(2)}` : '$0.00'
  const payoutDisplay  = winnerPayout ? `$${(winnerPayout / 100).toFixed(2)}` : potDisplay
  ...
  // Poll for match updates while pending (ready-up flow)
  useEffect(() => {
    if (!REAL_MATCH_ID || matchStatus !== 'pending') return
  ...
  const handleReadyUp = () => {
    if (!userTeamId || readyLoading) return
    setReadyLoading(true)
    matchesApi.readyUp(REAL_MATCH_ID, userTeamId).then((updated: any) => {
      setMatch(updated)
      ...
      <div style={{ maxWidth:1200, width:'100%', margin:'20px auto 0', padding:'0 16px', boxSizing:'border-box' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12, marginBottom:16 }}>
          <div style={{ display:'flex', alignItems:'center', gap:14 }}>
            <div style={{ background:'linear-gradient(135deg,rgba(212,146,10,.3),rgba(212,146,10,.15))', border:`1px solid ${ACCENT_BDR}`, boxShadow:`0 0 14px rgba(212,146,10,.2)`, borderRadius:8, padding:'6px 14px', fontSize:11, fontWeight:700, color:ACCENT, fontFamily:'Rajdhani, sans-serif', letterSpacing:.6, flexShrink:0 }}>
              WAGER MATCH
            </div>
            <div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            {/* Pot pill */}
            <div style={{ display:'flex', alignItems:'center', gap:8, background:ACCENT_DIM, border:`1px solid ${ACCENT_BDR}`, borderRadius:6, padding:'5px 14px' }}>
              <span style={{ fontSize:9, color:'#4F5568', fontFamily:'Rajdhani, sans-serif', fontWeight:700, letterSpacing:1, textTransform:'uppercase' }}>Pot</span>
              <span style={{ fontFamily:'Barlow Condensed, sans-serif', fontWeight:900, fontSize:18, color:ACCENT, lineHeight:1 }}>{potDisplay}</span>
            </div>
            {/* Match ID */}
        ...
        <div style={{ background:'#0F0F18', border:'1px solid rgba(255,255,255,.055)', borderRadius:10, overflow:'hidden', display:'grid', gridTemplateColumns:'1fr 300px 1fr' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', background:'#0B0B12', borderRight:'1px solid rgba(255,255,255,.055)' }}>
            ...
            <div style={{ margin:'12px 14px 4px', background:ACCENT_DIM, border:`1px solid ${ACCENT_BDR}`, borderRadius:6, padding:'10px 14px', textAlign:'center' }}>
              <div style={{ fontFamily:'Rajdhani, sans-serif', fontWeight:700, fontSize:10, color:'#4F5568', textTransform:'uppercase', letterSpacing:1, marginBottom:4 }}>
                {matchStatus === 'completed' ? 'Match Result' : 'Pot Breakdown'}
              </div>
              {matchStatus === 'completed' && match?.winnerId ? (
                <>
                  ...
                  <div style={{ fontFamily:'Barlow Condensed, sans-serif', fontWeight:900, fontSize:24, color:'#fff', lineHeight:1, marginBottom:6 }}>
                    {match.scoreA} - {match.scoreB}
                  </div>
                  <div style={{ fontFamily:'Barlow Condensed, sans-serif', fontWeight:900, fontSize:20, color:ACCENT, lineHeight:1 }}>{potDisplay}</div>
                  <div style={{ fontSize:9, color:'#8890A4', marginTop:4, fontFamily:'Rajdhani, sans-serif', lineHeight:1.5 }}>
                    Winner received ~<span style={{ color:'#4ade80', fontWeight:700 }}>{payoutDisplay}</span> after {platformFee}% fee
                  </div>
                </>
              ) : (
                <>
                  <div style={{ fontFamily:'Barlow Condensed, sans-serif', fontWeight:900, fontSize:22, color:ACCENT, lineHeight:1 }}>{potDisplay}</div>
                  <div style={{ fontSize:9, color:'#8890A4', marginTop:4, fontFamily:'Rajdhani, sans-serif', lineHeight:1.5 }}>
                    {entryDisplay} / player · {playerCount} players<br/>
                    Winner receives ~<span style={{ color:'#4ade80', fontWeight:700 }}>{payoutDisplay}</span> after {platformFee}% fee
                  </div>
                </>
              )}
            </div>
            ...
            <div style={{ padding:'4px 16px 12px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'4px 12px' }}>
              {[
                ...
                { label:'Entry',     val:`${entryDisplay} / player`, color:'#4ade80' },
            ...
            <div style={{ borderTop:'1px solid rgba(255,255,255,.055)', padding:'10px 16px', display:'flex', gap:8, flexWrap:'wrap', flexDirection:'column' }}>
              {/* Ready-Up for pending cash matches */}
              {matchStatus === 'pending' && userSide && match?.matchType === 'cash' && (
                <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                  {myTeamReady ? (
                    <div style={{ flex:1, padding:'10px 6px', background:'rgba(74,222,128,.08)', border:'1px solid rgba(74,222,128,.25)', borderRadius:7, fontSize:12, fontWeight:700, color:'#4ade80', fontFamily:'Rajdhani, sans-serif', letterSpacing:.4, textAlign:'center' }}>
                      {bothReady ? 'MATCH GOING LIVE...' : 'WAITING FOR OPPONENT...'}
                    </div>
                  ) : (
                    <button onClick={handleReadyUp} disabled={readyLoading} style={{ flex:1, padding:'10px 6px', background:'linear-gradient(135deg,#166534,#15803d)', border:'1px solid rgba(74,222,128,.4)', borderRadius:7, fontSize:13, fontWeight:800, color:'#fff', cursor:readyLoading?'wait':'pointer', fontFamily:'Rajdhani, sans-serif', letterSpacing:.6, textTransform:'uppercase', transition:'all .15s', boxShadow:'0 0 12px rgba(74,222,128,.2)' }}
                      onMouseEnter={e=>{if(!readyLoading)e.currentTarget.style.boxShadow='0 0 20px rgba(74,222,128,.35)'}}
                      onMouseLeave={e=>{e.currentTarget.style.boxShadow='0 0 12px rgba(74,222,128,.2)'}}>
                      {readyLoading ? 'READYING...' : 'READY UP'}
                    </button>
                  )}
                </div>
              )}
